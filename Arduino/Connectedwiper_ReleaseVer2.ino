...
#define PERIOD 5000 // milliseconds for each measurement
...
#define COUNT_THRESHOLD 8 // threshold of counting peaks of varXY and varZ between heavy rain and light rain 
...
int bluetoothTx = 4; // TX-0 pin of bluetooth to Arduino D2
int bluetoothRx = 5; // RX-1 pin of bluetooth to Arduino D3 
...
void loop()
{
  ...
  if (countVarXY > COUNT_THRESHOLD && countVarZ > COUNT_THRESHOLD) {
    bluetooth.println("Heavy Rain");
  } else if (countVarXY > 0 && countVarZ > 0) {
    bluetooth.println("Light Rain");
  } else {
    bluetooth.println("No Rain");
  }
}
